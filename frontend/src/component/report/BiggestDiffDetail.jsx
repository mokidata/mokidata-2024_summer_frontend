    ...
    if (todayArray.length !== 0 && lastArray.length !== 0) {
      ...
      todayArray.forEach((element) => {
        ...
        if (compareElement !== undefined) {
          ...
          const lastProfit = compareElement.price;
  ...
  return (
    ...
    >
      ...
      <div className="best-rank__button">
        <div
          className="best-rank__each-button"
          onClick={(event) => changeType("sale")}
        >
          <Button
            color={diffType === "sale" ? "black" : "white"}
            txt={props.t("biggestDiff.saleCount")}
          ></Button>
        </div>
        <div
          className="best-rank__each-button"
          onClick={(event) => changeType("profit")}
        >
          <Button
            color={diffType === "sale" ? "white" : "black"}
            txt={props.t("biggestDiff.saleProfit")}
          ></Button>
        </div>
      </div>
      <div className="best-rank__table">
        <div className="best-rank__column-info">
          ...
          <div className="best-rank__column" id="rank__last-profit">
            {props.page === "daily"
              ? props.t("biggestDiff.lastValue.daily")
              : props.page === "weekly"
              ? props.t("biggestDiff.lastValue.weekly")
              : props.page === "monthly"
              ? props.t("biggestDiff.lastValue.monthly")
              : ""}
          </div>
          <div className="best-rank__column" id="rank__profit">
            {props.page === "daily"
              ? props.t("biggestDiff.thisValue.daily")
              : props.page === "weekly"
              ? props.t("biggestDiff.thisValue.weekly")
              : props.page === "monthly"
              ? props.t("biggestDiff.thisValue.monthly")
              : ""}
          </div>
        </div>
        {diffType === "sale"

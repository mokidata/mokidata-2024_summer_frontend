...
function ReportPage(props) {
  ...
  const [componentFade, setComponentFade] = useState([
    true,
    false,
    false,
    false,
  ...
  const scrollToContent = (index) => {
    ...
    if (index >= 0 && index < items.length) {
      ...
      const elementPosition =
        items[index].getBoundingClientRect().top + window.scrollY;
    ...
    return (
      <div className="report-page">
        ...
        >
          <PredictSales
            currentDate={currentDate}
            // predictNextValue={predictNextValue}
            page={page}
            isVisible={componentFade[2]}
            value={predictValue}
            menuObject={menuObject}
            rankCompareValue={rankCompareValue}
            t={t}
            i18n={i18n}
          ></PredictSales>
        </motion.div>
